...
<div class="list_form_background">
    ...
    <?= $form->field($model, 'identity')->radioList(['商家'=>'商家','个人'=>'个人']) ?>
    ...
    $(document).ready(function(){
		...
		$("input[name=\"Info[identity]\"]").click(function(){
			
            var val = $(this).val();
			if(val == "商家"){
				$("textarea[name=\"Info[addr]\"]").parent().parent().show();
				$("textarea[name=\"Info[content]\"]").parent().parent().show();
			}else{
				$("textarea[name=\"Info[addr]\"]").parent().parent().hide();
				$("textarea[name=\"Info[content]\"]").parent().parent().hide();
			}
        });
